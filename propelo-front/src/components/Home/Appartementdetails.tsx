...
const AppartementDetails: React.FC = () => {
  ...
  useEffect(() => {
    const fetchApartmentDetails = async () => {
      try {
        const apartmentResponse = await axios.get(`http://localhost:5000/api/Apartment/${apartmentId}`);
        setApartment(apartmentResponse.data);

        const picturesResponse = await axios.get(`http://localhost:5000/api/ApartmentPicture/apartment/${apartmentId}`);
        setPictures(picturesResponse.data.map((pic: any) => pic.picturePath));

        const areasResponse = await axios.get(`http://localhost:5000/api/Apartment/areas/${apartmentId}`);
        setRooms(areasResponse.data.map((room: any) => ({ name: room.name, surface: room.surface })));

        const documentsResponse = await axios.get(`http://localhost:5000/api/ApartmentDocument/apartment/${apartmentId}`);
