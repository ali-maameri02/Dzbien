...
export const Home: React.FC = () => {
  ...
  useEffect(() => {
    const fetchApartments = async () => {
      try {
        const response = await axios.get('http://localhost:5000/api/Apartment');
        ...
        const picturePromises = unsoldApartments.map(async (apartment: any) => {
          const pictureResponse = await axios.get(`http://localhost:5000/api/ApartmentPicture/apartment/${apartment.id}`);
          return {
            id: apartment.id,
