...
const ApartmentsByProperty: React.FC = () => {
  ...
  useEffect(() => {
    const fetchProperty = async () => {
      ...
      try {
        const propertyResponse = await axios.get<Property>(`http://localhost:5000/api/Property/${propertyId}`);
        setProperty(propertyResponse.data);

        const pictureResponse = await axios.get<{ picturePath: string }[]>(`http://localhost:5000/api/PropertyPicture/property/${propertyResponse.data.id}`);
  ...
  useEffect(() => {
    const fetchApartments = async () => {
      ...
      try {
        const apartmentsResponse = await axios.get<Apartment[]>('http://localhost:5000/api/Apartment');
        ...
        const picturePromises = filtered.map(async (apartment) => {
          const pictureResponse = await axios.get<{ picturePath: string }[]>(`http://localhost:5000/api/ApartmentPicture/apartment/${apartment.id}`);
          return {
            id: apartment.id,
